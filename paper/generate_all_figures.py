...
# ============================================================================
# Data: M12 CRT-BOTH scaling
# ============================================================================
M12_CRT_DATA = [
    # (prime, crt_split, train_frac, best_acc, t_mem, t_90, t_99, total_ep)
    (113, '7x16', 0.30, 0.9988, 50, 9200, 23900, 32250),
    (113, '7x16', 0.60, 0.9995, 100, 100, 150, 5100),
    (241, '15x16', 0.30, 0.9996, 100, 100, 2600, 7550),
    (241, '15x16', 0.60, 1.0000, 100, 100, 100, 5050),
    (337, '16x21', 0.30, 0.9998, 100, 100, 250, 5200),
    (337, '16x21', 0.60, 1.0000, 100, 100, 150, 5100),
    (601, '24x25', 0.30, 1.0000, 150, 150, 150, 5100),
    (601, '24x25', 0.60, 1.0000, 350, 250, 400, 5350),
]
...
# ============================================================================
# Data: Large-prime scaling
# ============================================================================
LARGE_PRIME_DATA = [
    (113, 0.9981, 90, 26830, 33320, 'M04'),
    (257, 0.9753, 200, 137200, None, 'M13'),
    (521, 0.9993, 400, 13900, 19600, 'M14'),
]
...
# ============================================================================
# Figure 4: M12 CRT-BOTH scaling to larger primes
# ============================================================================
def fig_m12_crt_scaling():
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

    primes = [d[0] for d in M12_CRT_DATA]
    fracs = [d[2] for d in M12_CRT_DATA]
    accs = [d[3] * 100 for d in M12_CRT_DATA]
    t99 = [d[6] for d in M12_CRT_DATA]

    # Split by train fraction
    f30 = [(d[0], d[3]*100, d[6]) for d in M12_CRT_DATA if d[2] == 0.30]
    f60 = [(d[0], d[3]*100, d[6]) for d in M12_CRT_DATA if d[2] == 0.60]

    # Accuracy vs prime
    ax1.plot([d[0] for d in f30], [d[1] for d in f30], 'bs-', label='30% train', markersize=10)
    ax1.plot([d[0] for d in f60], [d[1] for d in f60], 'g^-', label='60% train', markersize=10)
    ax1.axhline(y=99, color='gray', linestyle='--', alpha=0.5)
    ax1.set_xlabel('Prime p')
    ax1.set_ylabel('Best Test Accuracy (%)')
    ax1.set_title('M12 CRT-BOTH: Accuracy vs Prime Size')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(95, 101)

    # T_99 vs prime
    ax2.plot([d[0] for d in f30], [d[2] for d in f30], 'bs-', label='30% train', markersize=10)
    ax2.plot([d[0] for d in f60], [d[2] for d in f60], 'g^-', label='60% train', markersize=10)
    ax2.set_xlabel('Prime p')
    ax2.set_ylabel('$T_{99}$ (epochs to 99% test acc)')
    ax2.set_title('M12 CRT-BOTH: Grokking Time vs Prime Size')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.set_yscale('log')

    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_m12_crt_scaling.pdf'), dpi=150, bbox_inches='tight')
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_m12_crt_scaling.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print('Generated fig_m12_crt_scaling.pdf')

# ============================================================================
# Figure 5: Prime-order DLP results
# ============================================================================
def fig_prime_order():
...
# ============================================================================
# Figure 6: Large-prime scaling
# ============================================================================
def fig_large_prime_scaling():
    fig, ax = plt.subplots(figsize=(8, 5))

    primes = [d[0] for d in LARGE_PRIME_DATA]
    accs = [d[1] * 100 for d in LARGE_PRIME_DATA]
    t90 = [d[3] for d in LARGE_PRIME_DATA]
    models = [d[5] for d in LARGE_PRIME_DATA]

    colors = ['#2ecc71' if a > 99 else '#f39c12' for a in accs]
    ax.scatter(primes, accs, c=colors, s=200, zorder=5, edgecolors='black')
    for i, m in enumerate(models):
        ax.annotate(f'{m}\n(p={primes[i]})', (primes[i], accs[i]), textcoords="offset points",
                    xytext=(10, 5), fontsize=9, fontweight='bold')
    ax.axhline(y=99, color='gray', linestyle='--', alpha=0.5, label='99% threshold')
    ax.set_xlabel('Prime p')
    ax.set_ylabel('Best Test Accuracy (%)')
    ax.set_title('Large-Prime Scaling (RAW representation)')
    ax.set_xscale('log')
    ax.legend()
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_large_prime_scaling.pdf'), dpi=150, bbox_inches='tight')
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_large_prime_scaling.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print('Generated fig_large_prime_scaling.pdf')


# ============================================================================
# Figure 7: Five-phase stepping board
# ============================================================================
def fig_five_phases():
