...
% Official JMLR style. The preprint option is appropriate for the initial submission/preprint draft.
...
\ShortHeadings{Grokking the Discrete Logarithm}{Vesterlund}
\firstpageno{1}

\begin{document}

\title{Grokking the Discrete Logarithm: Scaling Across Model Capacity and Mechanistic Analysis}

% JMLR requires the corresponding author's complete postal and e-mail address on the title page.
% The public/preprint version can remain email-free, but the JMLR submission copy must be completed.
\author{\name David Vesterlund \email davidsvens@gmail.com \\
\addr Industrial Research at Vesterlund Ventures Holding AB\\
Sweden\\
Sweden}
...
\begin{keywords}
grokking, algorithmic generalization, discrete logarithm, mechanistic interpretability, representation learning
\end{keywords}

\paragraph{Key findings.}
To the best of our knowledge, this is the first systematic grokking study of the full variable-base DLP mapping. A 427k-parameter transformer reaches 99.8\% test accuracy only after prolonged memorization. Across a 12-model sweep from 90k to 14.4M parameters, DLP grokking reappears at multiple neural scales, with a data-dependent non-monotonic region. Spectral and symbolic analyses identify multiplicative-character/Fourier structure, while prime-order and disjoint-generator controls preserve successful generalization.
...
The mathematical datasets in this study are deterministically generated from the task definitions. At preprint release, we will make the exact split manifests, run-level metrics, figure source data, model configurations, and training and analysis code publicly available in a versioned repository with an archival DOI. The release will include hashes for the principal checkpoints used in the mechanistic analyses so that the reported trajectories can be reproduced exactly.

\acks{
\textbf{Use of AI-assisted tools.} AI-assisted tools were used for programming assistance, code generation, language drafting, and editorial support. The research questions, hypotheses, experimental design, experiment selection, mathematical reasoning, code review, test execution and validation, result interpretation, proofreading, and final editing were carried out or directly verified by the author. All scientific claims and conclusions are the author's responsibility.

\textbf{Funding and competing interests.} [TO BE COMPLETED BEFORE SUBMISSION: disclose any third-party funding or support used for this work, including any LUMI/EuroHPC compute allocation, donated or subsidized compute, or programming support, as required by JMLR. Confirm any competing interests or state that none apply.]
}
